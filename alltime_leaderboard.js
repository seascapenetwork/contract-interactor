...
let getMintedAllTime = async function(sessionId) {
	let sql = `SELECT wallet_address, COUNT(nft_id) as amounts,updated_at FROM minted_leaderboards WHERE session_id = '${sessionId}' GROUP BY wallet_address ORDER BY COUNT(nft_id) DESC, MAX(updated_at) ASC LIMIT 10 `;
